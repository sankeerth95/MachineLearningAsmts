...
Xlist, ylist, p = utl.get_data("Train_features", "Train_labels")
X = np.array(utl.standardize(np.matrix(Xlist)))
y = mod_y(ylist)
...
Xlist, ylist, p = utl.get_data("Test_features", "Test_labels")
X = np.array(utl.standardize(np.matrix(Xlist)))

print "Test set parameters:\n", get_confusion_matrix(nx, X, ylist)
